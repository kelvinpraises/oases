...
export async function ensureNickFactory(publicClient: PublicClient, walletClient: WalletClient): Promise<void> {
...
const CONTRACTS_ROOT = resolve(currentDir, "..");
  ...
  for (const [file, libs] of Object.entries(linkReferences ?? {})) {
    for (const libName of Object.keys(libs)) {
      ...
      if (address === undefined) {
        throw new Error(`Artifact requires library "${fullyQualified}" but no address was provided for it`);
      }
...
export async function deployFromArtifact(
  ...
  const normalizedPath = artifactPath.startsWith("out/") ? artifactPath : join("out", artifactPath);
  const fullPath = join(CONTRACTS_ROOT, normalizedPath);
...
export function promoteDeployment(name: string): void {
  ...
  if (!existsSync(source)) throw new Error(`Cannot promote: missing deploy output at ${source}`);
  ...
  const snapshot = JSON.parse(readFileSync(dest, "utf-8")) as DeployState;
  writeDeploymentTs(name, snapshot);
}

function writeDeploymentTs(name: string, snapshot: DeployState): void {
  ...
  const ts = [
    ...
    'import type { EvmDeployOutput } from "../src/types.js";',
    "",
