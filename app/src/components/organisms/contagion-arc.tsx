      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-neutral-100 pb-4">
        <div className="space-y-0.5">
          ...
          <p className="text-xs text-neutral-500">
            Cascade dynamics across 4 Physical Classes: <code className="text-neutral-700 bg-neutral-100 px-1 py-0.5 rounded font-mono text-[11px]">Bond → Actor → Place → Act</code>
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 font-mono text-xs text-neutral-600 bg-neutral-50 px-3 py-1 rounded-lg border border-neutral-200">
          ...
          <span>{Math.max(1, sortedVaults.length - 1)} Linked Vectors</span>
        </div>
      </div>
          ...
          {sortedVaults.map((vault, index) => {
            ...
            return (
              <div
                ...
                className={`relative group cursor-pointer rounded-xl border p-4 transition-[border-color,box-shadow,background-color] duration-150 flex flex-col justify-between ${
                  isSelected
              ...
              >
                ...
                <div className="mt-4 space-y-3 pt-3 border-t border-neutral-100">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs font-mono">
                    <div>
                      <span className="text-neutral-400 text-[10px] block">Seed Base</span>
                      <span className="font-medium text-neutral-900">{formatUSDC(vault.seedPotUSDC)}</span>
                    </div>
                    <div>
                      <span className="text-neutral-400 text-[10px] block">Streamed</span>
                  ...
                  {!isLast && (
                    <div className="pt-2 flex items-center gap-1.5 text-[10px] font-mono text-amber-700 bg-amber-50/60 p-1.5 rounded border border-amber-200/60">
                      ...
                      <span className="truncate">{stepInfo?.vectorDescription || 'Contagion pressure'}</span>
      ...
      {activeVault && (
        <div className="rounded-lg bg-neutral-50 border border-neutral-200 p-3.5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs font-mono">
          ...
          <div className="flex items-center gap-3 text-neutral-600">
            <span>Base: <code className="text-neutral-800 font-semibold">$0.100 USDC</code></span>
            <span className="text-neutral-300">|</span>
            <span>Pot: <code className="text-neutral-800 font-semibold">{formatUSDC(activeVault.totalStreamedUSDC)}</code></span>
          </div>
        </div>
      )}
    </div>
